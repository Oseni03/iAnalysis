...
# snippet-end:[python.example_code.glue.GlueWrapper.decl]
    ...
    def create_crawler(self, name, role_arn, db_name, db_prefix, target):
        """
        ...
        :param db_prefix: The prefix to give any database tables that are created by
                          the crawler.
        :param target: The URL to an S3 bucket or database uri that contains data that is
        ...
        try:
            self.glue_client.create_crawler(
                ...
                Schedule=cron(15 12 * * ? *), # Can be changed later on
    ...
    def create_database(self, name):
        try:
            ...
            logger.error(
                "Couldn't create database %s. Here's why: %s: %s", name, err.response['Error']['Code'], err.response['Error']['Message'])
            raise
        else:
