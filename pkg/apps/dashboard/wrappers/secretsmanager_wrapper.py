...
# snippet-start:[python.example_code.secrets-manager.UpdateSecretVersionStage]
    def update_version_stage(self, name, stage, remove_from, move_to):
        """
        ...
        try:
            ...
            logger.info("Updated version stage %s for secret %s.", stage, name)
        except ClientError:
            logger.exception(
                "Couldn't update version stage %s for secret %s.", stage, name)
            raise
        else:
            return response
...
# snippet-start:[python.example_code.secrets-manager.DeleteSecret]
    def delete(self, name, without_recovery):
        """
        Deletes the secret.
        ...
        try:
            ...
            logger.exception("Deleted secret %s.", name)
            raise
